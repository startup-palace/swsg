...
final case object OpenApiConverter {
  def fromJson(json: String): Either[Seq[io.circe.Error], OpenApi] = {
    ...
    decodeAccumulating[Versions](json)
      .andThen { versions =>
        lazy val openApiVersionError = DecodingFailure(
          s"This tool only supports OpenAPI 3.0.x (current is ${versions.openapi})",
          List.empty)
        val openApiVersion =
          Validated.condNel(versions.openapi.substring(0, 3) == "3.0",
                            true,
                            openApiVersionError)
        val swsgVersionError = DecodingFailure(
          s"This tool only supports SWSG 1.0.x (current is ${versions.`x-swsg-version`})",
          List.empty)
        val swsgVersion =
          Validated.condNel(versions.`x-swsg-version`.substring(0, 3) == "1.0",
                            true,
                            swsgVersionError)
  ...
  def toModel(openapi: OpenApi): Either[Seq[String], Model] = {
    ...
    final object SchemaHasProps {
      def unapply(
          s: Schema): Option[(Map[String, SchemaOrRef], Set[String])] = {
    ...
    def parseSchemaReference(
        reference: String): ValidatedNel[String, String] = {
    ...
    def resolveSchemaReference(schemas: Map[String, SchemaOrRef])(
        reference: String): ValidatedNel[String, Schema] = {
    ...
    val schemas: Map[String, SchemaOrRef] = openapi.components
    ...
    val computeEntities: ValidatedNel[String, Set[Entity]] = {
      ...
      def flattenSchemas(allSchemas: Map[String, SchemaOrRef])(
          path: Seq[String],
          schemas: Seq[SchemaOrRef]): ValidatedNel[String, Schema] = {
        ...
        resolved.toVector.sequence
          .map(_.foldLeft(Schema.empty) {
            case (acc, cur) => acc merge cur
          })
      }

      def computeSchema(schemas: Map[String, SchemaOrRef])(
          path: Seq[String],
          schema: SchemaOrRef): ValidatedNel[String, Set[Variable]] =
        schema match {
          case OpenApi.Reference(ref) =>
            s"References are not supported (${path.mkString(" -> ")})".invalidNel
          ...
          case SchemaHasProps(properties, required) => {
            properties.toVector
              .map {
                case (name, SchemaHasType(t, format)) =>
                  toSwsgType(t).map { t =>
                    val computedType =
                      if (required.contains(name)) t else OptionOf(t)
                    Variable(name, computedType)
                  }
                case (name, _) =>
                  s"Type of ${(path :+ name).mkString(" -> ")} is a schema which is not supported".invalidNel
              }
              .sequence
              .map(_.toSet)
          }
          case _ =>
            s"The schema of '${path.mkString(" -> ")}' is not supported".invalidNel
        }

      def computeEntity(schemas: Map[String, SchemaOrRef])(
          name: String,
          schema: SchemaOrRef): ValidatedNel[String, Entity] = {
        computeSchema(schemas)(Seq(name), schema).map(attrs =>
          Entity(name, attrs))
      }

      val computedEntities: Vector[ValidatedNel[String, Entity]] =
        schemas.toVector
          .map(s => computeEntity(schemas)(s._1, s._2))

      computedEntities.sequence
        .map(_.toSet)
    }
    ...
    val computeServices: ValidatedNel[String, Seq[Service]] = {
      ...
      def extractParameters(
          o: Operation): ValidatedNel[String, Set[ServiceParameter]] = {
        ...
        val extractedBody = o.requestBody
          .map(b => extractBody(b).map(sp => Set(sp)))
          .getOrElse(Set.empty.validNel)
        (extractedParameters, extractedBody).mapN(_ ++ _)
      }

      def extractBody(
          b: RequestBodyOrRef): ValidatedNel[String, ServiceParameter] = {
        b match {
          case OpenApi.Reference(_) =>
            "References in operation requestBody are not handled".invalidNel
          case RequestBody(_, _, _, None) =>
            "RequestBody in operation must have a 'x-swsg-name' attribute".invalidNel
          case RequestBody(_, content, required, Some(name)) => {
            if (content.size != 1) {
              "Multiple items in content in RequestBody are not handled".invalidNel
            } else {
              val extractedType = content.values.head.schema
                .map(
                  s =>
                    schemaToType(Seq("RequestBody"),
                                 s,
                                 required.getOrElse(false)))
                .getOrElse(
                  "Content items in RequestBody must have a 'schema' attribute".invalidNel)
      ...
      def extractParameter(
          p: ParameterOrRef): ValidatedNel[String, ServiceParameter] = {
        p match {
          case OpenApi.Reference(_) =>
            "References in operation parameters are not handled".invalidNel
          case p @ Parameter(name, in, _, _, _, _, _, _, _, _, _) => {
            ...
            supported.get(in) match {
              case Some(location) =>
                extractParameterType(p).map(t =>
                  ServiceParameter(location, Variable(name, t)))
              case None =>
                s"Value in = '$in' of parameter $name is not supported. It must be one of ${supported
                  .mkString(", ")}.".invalidNel
            }
          }
        }
      ...
      def extractParameterType(p: Parameter): ValidatedNel[String, Type] = {
        ...
        p.schema match {
          case None =>
            s"A schema must be specified in parameter '${p.name}'".invalidNel
          case Some(s) =>
            schemaToType(Seq(s"parameter '${p.name}'"), s, required)
        }
      }

      def schemaToType(parents: Seq[String],
                       schemaOrRef: SchemaOrRef,
                       required: Boolean): ValidatedNel[String, Type] = {
        val computedType: ValidatedNel[String, Type] = schemaOrRef match {
          case OpenApi.Reference(ref) =>
            parseSchemaReference(ref).andThen(n => EntityRef(n).valid)
          ...
          case s @ SchemaHasType("array", _) =>
            s.items match {
              case Some(sub) =>
                schemaToType(parents :+ "[list]", sub, true).map(computedSub =>
                  SeqOf(computedSub))
              case None =>
                s"Schemas of type 'array' must have an 'items' attribute in '${parents
                  .mkString(" -> ")}'".invalidNel
            }
          case SchemaHasType("null", _) =>
            s"'null' type are not allowed in '${parents.mkString(" -> ")}'".invalidNel
          case _ =>
            s"Schema is not supported in '${parents.mkString(" -> ")}'".invalidNel
        }
        computedType.map { t =>
          schemaOrRef match {
      ...
      val operations: Seq[(String, String, Operation)] = openapi.paths.toVector
        .flatMap {
          case (path, pathItem) =>
      ...
      val swsgOperations: Seq[(String, String, ComponentInstance, Operation)] =
        operations.flatMap {
          case (method,
                path,
                o @ Operation(_, _, _, _, _, _, _, _, _, Some(ci))) =>
            Vector((method, path, ci, o))
          case (_, _, Operation(_, _, _, _, _, _, _, _, _, None)) =>
            Vector.empty
        }
