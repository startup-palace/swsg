  ...
  final case class Schema(
    ...
    def merge(s2: Schema): Schema = {
      Schema(
        ...
        this.required
          .map(p1 => p1 ++ s2.required.getOrElse(Set.empty))
          .orElse(s2.required),
        this.enum.map(p1 => p1 ++ s2.enum.getOrElse(Seq.empty)).orElse(s2.enum),
        s2.`type`.orElse(this.`type`),
        this.allOf
          .map(p1 => p1 ++ s2.allOf.getOrElse(Seq.empty))
          .orElse(s2.allOf),
        this.oneOf
          .map(p1 => p1 ++ s2.oneOf.getOrElse(Seq.empty))
          .orElse(s2.oneOf),
        this.anyOf
          .map(p1 => p1 ++ s2.anyOf.getOrElse(Seq.empty))
          .orElse(s2.anyOf),
        this.not.map(p1 => p1 ++ s2.not.getOrElse(Seq.empty)).orElse(s2.not),
        this.items
          .map(p1 => p1 merge s2.items.getOrElse(Schema.empty))
          .orElse(s2.items),
        this.properties
          .map(p1 => p1 ++ s2.properties.getOrElse(Map.empty))
          .orElse(s2.properties),
