...
final case object OpenApiInstances {
  ...
  implicit val decodePathItem: Decoder[PathItem] = new Decoder[PathItem] {
    final def apply(c: HCursor): Decoder.Result[PathItem] = {
      def getOperations(c: HCursor, fields: Vector[String]): Decoder.Result[ListMap[String, Operation]] = {
        val empty: Decoder.Result[ListMap[String, Operation]] = Right(ListMap.empty)
        val operations = fields
          ...
          .foldLeft(empty) {
            ...
            case (Right(acc), (method, Right(op))) => Right(acc + (method -> op))
          }
        operations
      }
      ...
      for {
        ...
        fields <- c.keys.map(_.toVector).toRight(DecodingFailure("Current element should be an object", c.history))
